...
import { CheemsBonkOverlay, type BonkEvent } from '@/components/CheemsBonkOverlay'
...
export const InventoryPage: React.FC = () => {
  ...
  const [bonkEvents, setBonkEvents] = useState<BonkEvent[]>([])
  const [bonkingIds, setBonkingIds] = useState<string[]>([])

  const triggerCheemsBonk = useCallback((targetX: number, targetY: number) => {
    const newEvent: BonkEvent = {
      id: Date.now() + Math.floor(Math.random() * 1000),
      x: targetX,
      y: targetY,
      createdAt: Date.now()
    }
    setBonkEvents((prev) => [...prev, newEvent])
    setTimeout(() => {
      setBonkEvents((prev) => prev.filter((ev) => ev.id !== newEvent.id))
    }, 1400)
  }, [])
  ...
  const toggleFavorite = useCallback(
    (e: React.MouseEvent, id: string) => {
      e.stopPropagation()
      if (bonkingIds.includes(id)) return
      ...
      if (isBecomingFavorite) {
        ...
        setFilaments((prev: Filament[]) => prev.map((f) => (f.id === id ? { ...f, isFavorite: true } : f)))
      } else if (isMobile) {
        // Trên Mobile (< 768px): Chạy hiệu ứng Cheems Bonk
        triggerCheemsBonk(clickX, clickY)
        setBonkingIds((prev) => [...prev, id])

        // Delay removing color/favorite until Cheems completes the swing (~750ms)
        setTimeout(() => {
          setFilaments((prev: Filament[]) => prev.map((f) => (f.id === id ? { ...f, isFavorite: false } : f)))
          setBonkingIds((prev) => prev.filter((itemId) => itemId !== id))
        }, 750)
      } else {
        // Trên Desktop (>= 768px): Không chạy GIF Cheems, bỏ yêu thích ngay lập tức
        setFilaments((prev: Filament[]) => prev.map((f) => (f.id === id ? { ...f, isFavorite: false } : f)))
      }
    },
    [filaments, setFilaments, triggerHeartBurst, triggerCheemsBonk, bonkingIds]
  )
  ...
  return (
    <div className='relative overflow-x-clip space-y-4 sm:space-y-6 animate-in fade-in duration-300 max-w-6xl mx-auto'>
      ...
      {filaments.length === 0 ? (
        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4.5'>
          {displayedFilaments.map((item: Filament) => {
            ...
            const isBonking = bonkingIds.includes(item.id)
            const isFav = !!item.isFavorite || isBonking
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className={`absolute top-2.5 right-2.5 z-20 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 cursor-pointer active:scale-90 ${isBonking ? 'animate-heart-bonked' : ''} ${
                    isFav
                ...
                <div>
                  ...
                  <div className='mb-3'>
                    ...
                    <div className='flex items-center gap-1.5 flex-wrap mt-1'>
                      ...
                      <span className='text-[10px] sm:text-[11px] opacity-60 truncate'>{item.colorName}</span>
                    </div>
                    {item.notes && (
                ...
                >
                  <div className='col-span-4 flex items-center gap-3'>
                    ...
                    <button
                      ...
                      onClick={(e) => toggleFavorite(e, item.id)}
                      title={isFav ? 'Bỏ yêu thích' : 'Thêm vào yêu thích'}
                      className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 cursor-pointer active:scale-90 ${isBonking ? 'animate-heart-bonked' : ''} ${
                        isFav
                    ...
                    >
                      <Heart size={14} className={`transition-all ${isFav ? 'fill-white' : ''}`} />
                    </button>
                  </div>
      ...
      {/* Hiệu ứng Cheems Bonk khi bỏ yêu thích */}
      <CheemsBonkOverlay events={bonkEvents} />

      <style>{`
                @keyframes heartBonkShake {
                    0%, 35% {
                        transform: scale(1.05);
                    }
                    42% {
                        transform: scale(0.6, 1.45) rotate(-20deg);
                    }
                    50% {
                        transform: scale(1.4, 0.6) rotate(18deg);
                    }
                    60% {
                        transform: scale(0.85, 1.15) rotate(-10deg);
                    }
                    72% {
                ...
                .animate-heart-bonked {
                    animation: heartBonkShake 0.75s cubic-bezier(0.36, 0.07, 0.19, 0.97) forwards !important;
