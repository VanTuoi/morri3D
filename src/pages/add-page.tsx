...
import { z } from 'zod'
import { PlusCircle, X, ShoppingBag, Layers, Check, AlertTriangle } from 'lucide-react'
...
const orderSchema = z.object({
            ...
            z.object({
                ...
                weight: z
                    .preprocess(
                        ...
                        z.number().min(0, 'Số gam không được âm')
                    )
                    .optional()
            })
        )
    ...
    price: z
        .preprocess(
            (val) => (val === '' || val === undefined || val === null ? 0 : Number(val)),
            z.number().min(0, 'Thành tiền không được âm')
        )
        .optional(),
...
export const AddPage: React.FC = () => {
    ...
    const {
        ...
        defaultValues: {
            ...
            address: '',
            itemName: '',
            materials: [{ inventoryId: '', type: '', color: '', weight: '' as any }],
            quantity: 1,
            price: '' as any,
            notes: ''
        }
    ...
    const onOrderSubmit = (data: OrderFormValues) => {
        const orderToAdd: Order = {
            ...
            materials: data.materials.map((m) => ({
                ...m,
                weight: m.weight !== undefined && m.weight !== null && m.weight !== ('' as any) ? Number(m.weight) : 0
            })),
                    ...
                    <form onSubmit={handleSubmitOrder(onOrderSubmit)} className='space-y-3.5 sm:space-y-4'>
                        ...
                        <div className='grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-3.5'>
                                ...
                                <Input
                                    ...
                                    className={
                                        orderErrors.customerName
                                            ? 'border-rose-500/80 focus-visible:ring-rose-500/30'
                                            : ''
                                    }
                            ...
                            <div>
                                ...
                                <Input
                                    type='tel'
                                    className={
                                        orderErrors.phone ? 'border-rose-500/80 focus-visible:ring-rose-500/30' : ''
                                    }
                                    {...registerOrder('phone')}
                                />
                                {orderErrors.phone && (
                        ...
                        <div className='pt-2 border-t border-inherit/40'>
                            ...
                            <Input
                                type='text'
                                className={
                                    orderErrors.itemName ? 'border-rose-500/80 focus-visible:ring-rose-500/30' : ''
                                }
                                {...registerOrder('itemName')}
                            />
                            {orderErrors.itemName && (
                                <span className='text-[10px] text-rose-500 font-medium mt-1 block'>
                                    {orderErrors.itemName.message}
                                </span>
                            )}
                        </div>
                        ...
                        <div className='space-y-2.5 pt-1'>
                            <div className='flex items-center justify-between'>
                                <label className='block text-[11px] sm:text-xs font-semibold opacity-70'>
                                    Lựa chọn nhựa in
                                </label>
                                {materialFields.length > 0 && (
                                    <span className='text-[10px] opacity-60 font-medium'>
                                        ({materialFields.length} loại nhựa)
                                    </span>
                                )}
                            </div>
                            ...
                            {materialFields.map((field, index) => {
                                ...
                                const selectedFil = filaments.find((f: Filament) => f.id === currentInventoryId)
                                const availableWeight = selectedFil
                                    ? (selectedFil.weight ??
                                      (selectedFil.percentage !== undefined ? selectedFil.percentage * 10 : 1000))
                                    : 0
                                const enteredWeight = Number(watchMaterials?.[index]?.weight || 0)
                                const isExceeded = !!selectedFil && enteredWeight > 0 && enteredWeight > availableWeight
                                    ...
                                    >
                                        ...
                                        <FilamentSelect
                                            ...
                                            onChange={(val) => {
                                                if (val) {
                                                    ...
                                                    if (fil) {
                                                        setOrderValue(`materials.${index}.inventoryId`, val, {
                                                            shouldValidate: true
                                                        })
                                                        setOrderValue(
                                                    ...
                                                    setOrderValue(`materials.${index}.inventoryId`, '', {
                                                        shouldValidate: true
                                                    })
                                        ...
                                        <div className='grid grid-cols-2 gap-2.5 pt-1'>
                                            <div>
                                                ...
                                                <div className='relative'>
                                                    <Input
                                                        ...
                                                        step='any'
                                                        placeholder=''
                                                        className={`h-10 font-black pr-8 ${isExceeded ? 'border-amber-500/70 focus-visible:ring-amber-500/30' : ''}`}
                                                        {...registerOrder(`materials.${index}.weight` as const)}
                                                    />
                                            ...
                                            <div>
                                                ...
                                                <div className='w-full h-10 px-3 rounded-xl border border-zinc-200/80 dark:border-white/10 bg-zinc-100/70 dark:bg-white/[0.03] flex items-center justify-between text-xs'>
                                        ...
                                        {selectedFil && enteredWeight > 0 && (
                                            <div className='flex items-center justify-between text-[10px] sm:text-[11px] px-1 opacity-70'>
                                        ...
                                        {isExceeded && (
                                            <div className='p-2 rounded-xl border border-amber-500/40 bg-amber-500/10 text-amber-700 dark:text-amber-300 text-[11px] flex items-start gap-2 animate-in fade-in-50 duration-200'>
                                                ...
                                                <div className='leading-tight'>
                                                    ...
                                                    Số gam chọn (<strong>{enteredWeight}g</strong>) vượt quá lượng còn
                                                    lại trong cuộn (<strong>{availableWeight}g</strong>).
                                                </div>
                                            </div>
                                        )}
                            ...
                            <button
                                type='button'
                                onClick={() =>
                                    appendMaterial({ inventoryId: '', type: '', color: '', weight: '' as any })
                                }
                    ...
                    <form onSubmit={handleSubmitFilament(onFilamentSubmit)} className='space-y-3.5 sm:space-y-4'>
                        <div className='flex items-center gap-2 pb-2 border-b border-inherit/40'>
                            ...
                            <h2 className='text-xs sm:text-sm font-bold tracking-tight'>
                                Nhập thông tin cuộn nhựa vào kho
                            </h2>
                        </div>
                        ...
                        <div className='pt-2 border-t border-inherit/40 space-y-2.5'>
                            ...
                            <div className='flex flex-wrap items-center gap-2'>
                                ...
                                >
                                    {!BASIC_COLORS.some(
                                        (c) => c.hex.toLowerCase() === watchColorHex?.toLowerCase()
                                    ) && (
                                        <Check
                                            size={13}
                                            className='text-white drop-shadow-md mx-auto z-10'
                                            strokeWidth={3}
                                        />
                                    )}
                                    <input
                        ...
                        {pendingVariations.length > 0 && (
                            <div className='space-y-1.5 pt-2 border-t border-inherit/40'>
                                <div className='text-[11px] font-bold opacity-70'>
                                    Danh sách cuộn sẽ nhập (
                                    {pendingVariations.reduce((acc: number, curr: any) => acc + curr.quantity, 0)}{' '}
                                    cuộn):
                                </div>
