...
export function useManagerData() {
    ...
    try {
        ...
        sessionStorage.removeItem('3dManager_orders')
        sessionStorage.removeItem('3dManager_filaments')
    } catch {
        // ignore
    }
            ...
            if (response.credential) {
                ...
                if (decoded) {
                    ...
                    if (normalizedAllowed.length > 0) {
                        ...
                        setAuthError(
                            `Không tìm thấy danh sách email được cấp phép trong tab "Users" trên Google Sheet.`
                        )
                        return
                    }
            ...
            try {
                ...
                if (res.success && res.data) {
                    ...
                    if (Array.isArray(res.data.allowedEmails)) {
                        ...
                        const normEmails = res.data.allowedEmails
                    ...
                    setSyncStatus('error')
                    setSyncMessage(res?.error || 'Lỗi đọc dữ liệu từ Google Sheets')
                }
            } catch (err: any) {
                ...
                setSyncMessage(
                    err?.message
                        ...
                        : 'Không thể kết nối tới Google Apps Script URL. Vui lòng kiểm tra lại mạng hoặc URL.'
                )
            }
        },
        [gasUrl, user]
    ...
    const handleSaveGasUrl = (newUrl?: string) => {
        const targetUrl = typeof newUrl === 'string' ? newUrl : tempGasUrl
        const trimmed = targetUrl.trim()
        setGasUrl(trimmed)
        setTempGasUrl(trimmed)
