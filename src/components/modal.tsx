                ...
                {showDeleteConfirm && (
                    ...
                    >
                        ...
                        <p className={`text-center mb-8 text-sm px-2 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                            ...
                            <span className='text-rose-500 dark:text-rose-300 font-bold'>#{order.id}</span> không? Hành
                                    ...
                                    {(order.materials && order.materials.length > 0
                                        ...
                                        const fil = filaments.find(
                                            (f) =>
                                                ...
                                                (f.colorName &&
                                                    m.color &&
                                                    f.colorName.toLowerCase() === m.color.toLowerCase())
                                        )
                                        ...
                                        const weight =
                                            fil !== undefined
                                                ? (fil.weight ??
                                                  (fil.percentage !== undefined ? fil.percentage * 10 : 1000))
                                                : null
...
export const FilamentModal: React.FC<FilamentModalProps> = ({
    ...
    const availableTypes = Array.from(
        new Set([
            ...['PLA Matte', 'PLA Basic', 'PLA Silk', 'PLA Lite', 'PETG Matte', 'PETG Basic'],
            editingFilament.type
        ])
    )
                    ...
                    {gasUrl && (
                        ...
                        >
                            <div className='flex items-center gap-2.5'>
                                <RefreshCw
                                    size={15}
                                    className={`text-orange-500 ${syncStatus === 'syncing' ? 'animate-spin' : ''}`}
                                />
                                <div>
